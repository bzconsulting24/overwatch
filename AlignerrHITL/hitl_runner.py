...
class HITLRunner:
    ...
    def run_analysis(self, video_url,
        ...
        # 1) clear folder
        if status_callback:
            status_callback("Clearing output folder...")
        ...
        # 4) speech features
        if status_callback:
            status_callback("Extracting speech features...")
        ...
        if status_callback:
            status_callback("Analyzing behavior summary...")
        ...
        # 6) OpenFace
        if status_callback:
            status_callback("Running OpenFace analysis...")
        runOpenface(video_file, self.output_dir)
        if progress_callback:
            progress_callback(60)

        if status_callback:
            status_callback("Processing OpenFace results...")
        openface_summary = analyze_behavior(video_file, self.output_dir)
        if progress_callback:
            progress_callback(70)

        # 7) transcribe & extra analyses
        if status_callback:
        ...
        # 8) append analyses to transcript.txt
        ...
        # 9) read and return full transcript
